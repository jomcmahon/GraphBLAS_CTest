...
bool run_MAssn(testargs *myargs)
{
  ...
  TEST_OK(read_typed_matrix(myargs->inbase, myargs->input0, thetype, &A));
...
void default_spec(testargs *myargs, int **specptr)
{
  ...
  if (strlen(myargs->input0) == 0) strcpy(myargs->input0, "testMExtr/C");
...
void default_specA(testargs *myargs, int **specptr)
{
  ...
  if (strlen(myargs->input0) == 0) strcpy(myargs->input0, "testMExtr/CA");
...
void default_specR(testargs *myargs, int **specptr)
{
  ...
  if (strlen(myargs->input0) == 0) strcpy(myargs->input0, "testMExtr/CR");
...
void default_specS(testargs *myargs, int **specptr)
{
  ...
  if (strlen(myargs->input0) == 0) strcpy(myargs->input0, "testMExtr/CS");
...
void default_specB(testargs *myargs, int **specptr)
{
  ...
  if (strlen(myargs->input0) == 0) strcpy(myargs->input0, "testMExtr/CB");
...
int main(int argc, char * argv[])
{
  ...
  testerror = test_L_DA_loop(myargs, TYPE, specptr, run_MAssn);
  testerror |= test_L_DA_loop(&myargsA, TYPE, specptrA, run_MAssn);
  testerror |= test_L_DA_loop(&myargsR, TYPE, specptrR, run_MAssn);
  testerror |= test_L_DA_loop(&myargsS, TYPE, specptrS, run_MAssn);
  testerror |= test_L_DA_loop(&myargsB, TYPE, specptrB, run_MAssn);
