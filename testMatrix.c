...
#define SET_AND_TEST							\
    OK (GrB_Matrix_new(&mat, thetype, nr, nc));			\
    TEST_MAT(mat);							\
    OK (GrB_Matrix_setElement(mat, s, rind, cind));			\
    OK (GrB_Matrix_setElement(mat, s, rind + 1, cind + 1));		\
    nv += 2; TEST_ELTS(mat);						\
    nr++; nc++;	OK (GxB_Matrix_resize(mat, nr, nc));		\
    TEST_ELTS(mat);							\
    OK (GrB_Matrix_extractTuples(NULL, NULL, X, &nv, mat));		\
    bool vtest = ((X[0] == r) && (X[1] == r));				\
    SUBTEST_COND(vtest, "extracted tuples not correct");		\
    OK (GrB_Matrix_extractTuples(NULL, J, X, &nv, mat));		\
    vtest = ((X[0] == r) && (X[1] == r) && (J[0] == cind) &&		\
	     (J[1] == cind + 1));					\
    SUBTEST_COND(vtest, "extracted tuples not correct");		\
    OK (GrB_Matrix_extractTuples(I, J, X, &nv, mat));		\
